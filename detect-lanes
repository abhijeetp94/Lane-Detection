...
def CannyEdge(image):
    ...
    up_yellow = np.array([135, 60, 255])
    mask = cv2.inRange(hsv, low_yellow, up_yellow)
    cannyImage = cv2.Canny(mask, 50, 150)
    return cannyImage


def region_of_interest(image):
    height = image.shape[0]
    triangle = np.array([[(0, height-50),(300, 100),(600, height-50),]], np.int64)
...
while(cap.isOpened()):
    ...
    lines = cv2.HoughLinesP(cropped_Image,rho, theta, threshold, np.array ([ ]), minLineLength=100, maxLineGap=50)
    ...
    cv2.imshow("Interest", cropped_Image)
